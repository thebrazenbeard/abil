...
Schema validity is structural only. Stateful write admission additionally requires independently rooted current-authority evidence whose freshness deadlines are independently derived from a bound profile, whose current/revoked/superseded epoch state is materialized and digest-bound under an independently resolved canonicalization profile so the validator can recompute the comparison maximum, and whose reconciled authority-surface set is mechanically checked against an independently resolved deployment-required set. Summary labels such as `FRESH` or `COMPLETE` never substitute for those computations.
